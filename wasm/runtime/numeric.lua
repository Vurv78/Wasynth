...
local function from_u32(low, high) return setmetatable({low, high}, Numeric) end

local function to_u32(value) return value[1], value[2] end
...
local function op_add(lhs, rhs)
	...
	if high >= N_2_TO_32 then high = high - N_2_TO_32 end
...
local function op_subtract(lhs, rhs)
	...
	if high < 0 then high = high + N_2_TO_32 end
...
local function op_multiply(lhs, rhs)
	if op_is_zero(lhs) or op_is_zero(rhs) then return VAL_ZERO end
	...
	if op_is_less_unsigned(lhs, VAL_2_TO_24) and op_is_less_unsigned(rhs, VAL_2_TO_24) then
		...
		if has_negative then result = op_negate(result) end
...
local function op_divide_unsigned(lhs, rhs)
	if op_is_zero(lhs) then
		return 0
	elseif op_is_zero(rhs) then
	...
	while op_is_greater_unsigned(rem, rhs) or op_is_equal(rem, rhs) do
		...
		if op_is_zero(res_temp) then res_temp = VAL_ONE end
...
function op_negate(value) return op_add(op_bnot(value), VAL_ONE) end
...
function Numeric.__le(lhs, rhs) return op_is_less_unsigned(lhs, rhs) or op_is_equal(lhs, rhs) end

function Numeric.__tostring(value) return tostring(to_f64(value)) end
